"""Path-A: run our MSA-Qwen3.5-9B SFT-S2 ckpt on EverMind paper Table 1's
9 datasets, output EverMind-compatible JSON for downstream LLM-judge scoring.

For each bench:
  - load full corpus from /workspace/encoded_corpora/<bench>_full or _99999
  - sample N queries (eligible = gold refs all in corpus)
  - retrieve() -> router top-k doc ids        (EverMind: pred_id)
  - generate() -> parse answer string         (EverMind: pred_answer)
  - dump JSON {anonymous: {precision: {metrics: ..., record_list: [...]}}}
"""
...
# Reuse already-trusted EverMind benchmark pickle files via lazy import.
_pkl = __import__("pickle")
...
def run_bench(bench, num_queries, top_k, args, model, tok, device):
    ...
    final = {"anonymous": {"precision": {"metrics": metrics, "record_list": record_list}}}
...
def main():
    ...
    ap.add_argument("--out_dir", default="/workspace/eval_evermind_aligned")
    ...
    print(
        f"[evermind-aligned] benches={benches}  "
        f"N={args.num_queries}/each  top_k={args.top_k}"
    )
    summary = {}
    t0 = time.time()
    for b in benches:
        m = run_bench(b, args.num_queries, args.top_k, args, model, tok, device)
        if m:
            summary[b] = m
    ...
    with open(Path(args.out_dir) / "summary.json", "w") as f:
        json.dump({"benches": summary, "args": vars(args)}, f, indent=2)
